...
class WC_Subscriptions_Manager {
	...
	public static function init() {
		...
		// Assign the inactive role to a user until payment is completed for a subscription
		add_action( 'scheduled_subscription_payment', __CLASS__ . '::put_subscription_on_hold', 1, 2 );
	...
	public static function can_subscription_be_changed_to( $new_status, $subscription_key, $user_id = '' ) {
		...
		if ( empty( $subscription ) ) {
			...
			switch( $new_status ) {
				case 'active' :
					if ( ( $order_uses_manual_payments || $payment_gateway->supports( 'subscription_reactivation' ) ) && $subscription['status'] != 'active' && $subscription['status'] == 'on-hold' )
		...
		foreach ( $subscriptions_to_search as $user_id => $subscriptions ) {
			...
			if ( false != preg_match( "/$search_query/i", $user->user_nicename ) || false != preg_match( "/$search_query/i", $user->display_name ) ) {
				...
				foreach ( $subscriptions as $subscription_key => $subscription ) {
					...
					if ( in_array( $search_query, $subscription ) || false != preg_match( "/$search_query/i", $product_title ) )
