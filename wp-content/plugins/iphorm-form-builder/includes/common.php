...
{
    $testDate = strtotime('25th December 2012 17:35');
    $dateFormats = array();
    $formats = array(
        'l, jS F Y',
        'D, jS M Y',
...
function iphorm_process_form()
{
    ...
    if (isset($_POST['iphorm_id']) && isset($_POST['iphorm_uid']) && (($form = iphorm_get_form($_POST['iphorm_id'], $_POST['iphorm_uid'])) instanceof iPhorm) && $form->getActive()) {
        ...
        // Calculate which elements are hidden by conditional logic
        if ($form->hasConditionalLogic()) {
            $form->setConditionallyHiddenElements();
        }
        ...
        if ($form->isValid()) {
            ...
            if ($form->getSendNotification() || $form->getSendAutoreply()) {
                ...
                if ($form->getSendNotification() && count($form->getRecipients())) {
                    ...
                    $emailHTML = apply_filters('iphorm_email_body', $emailHTML, $form);
                    $emailHTML = apply_filters('iphorm_email_body_' . $form->getId(), $emailHTML, $form);

                    $emailPlain = apply_filters('iphorm_autoreply_body', $emailPlain, $form);
                    $emailPlain = apply_filters('iphorm_autoreply_body_' . $form->getId(), $emailPlain, $form);
...
function iphorm_enqueue_styles()
{
    ...
    if (!get_option('iphorm_disable_qtip_output')) {
        wp_enqueue_style('qtip', IPHORM_PLUGIN_URL . '/js/qtip2/jquery.qtip.min.css', array(), '2.0.0');
    }
...
function iphorm_enqueue_scripts()
{
    ...
    if (!get_option('iphorm_disable_qtip_output')) {
        wp_enqueue_script('qtip', IPHORM_PLUGIN_URL . '/js/qtip2/jquery.qtip.min.js', array('jquery'), '2.0.0', true);
    }
