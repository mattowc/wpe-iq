...
function iphorm_activate()
{
    ...
    add_option('iphorm_last_update_check', 0);
...
function iphorm_uninstall()
{
    ...
    delete_option('iphorm_last_update_check');
        ...
        if (wp_remote_retrieve_response_code($remoteResponse) == 200 && strlen($json = wp_remote_retrieve_body($remoteResponse))) {
            ...
            if (isset($data['type'])) {
                if ($data['type'] == 'success') {
                    ...
                    update_option('iphorm_last_update_check', 0);
...
function iphorm_update_check($transient)
{
    ...
    $lastUpdateCheck = get_option('iphorm_last_update_check');
    $time = time();
    $hourAgoTime = $time - (1 * 60 * 60);

    $tooSoon = ($lastUpdateCheck > $hourAgoTime) && apply_filters('iphorm_throttle_update_check', true);

    if (empty($transient->checked) || !strlen($licenceKey) || $tooSoon) {
        return $transient;
    }
    ...
    update_option('iphorm_last_update_check', $time);
...
/**
 * Enqueue the datepicker on the admin export page
 */
                            ...
                            if (isset($elementsCache[$elementId])) {
                                ...
                                if (isset($elementsCache[$elementId]['type'])) {
                                    switch ($elementsCache[$elementId]['type']) {
                                        ...
                                        case 'textarea':
                                            $row[$field] = preg_replace('/<br\s*?\/>/', '', $row[$field]);
                                            break;
                                        case 'email':
